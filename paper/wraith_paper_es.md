...
8. **Eficiencia práctica**: empaquetado sin pérdida a 74.9 MB (compresión 4.97×), inferencia en CPU a 52.1 tok/s mediante motor C++ AVX2, y — bajo el supuesto de que la convergencia exponencial de fp16 continúe más allá de nuestra medición a 1.6B tokens — un costo estimado 11.2× menor para alcanzar calidad equivalente. Este factor depende de una extrapolación de curva y se discute con caveats en Sec. 4.4.
...
**Si este escalado se sostiene, es potencialmente el hallazgo de mayor impacto del paper.** En la era actual donde los grandes laboratorios de IA se enfrentan a la escasez de datos de texto de calidad para entrenamiento (Villalobos et al., 2024; Muennighoff et al., 2023), un paradigma con la eficiencia por token de Wraith podría reducir la demanda de datos de entrenamiento, que es actualmente el cuello de botella económico y físico del scaling de LLMs.

**Qué está medido, qué no lo está.** A 186M, Wraith alcanza val PPL 107 con 1.6B tokens, mientras el baseline fp16 LLaMA arquitectura-idéntico queda en val PPL 614 tras los mismos 1.6B tokens — una ventaja **medida** de 5.73× a presupuesto idéntico (Sec. 4.2). El claim "~20-30× menos tokens que BitNet y ~2-3× menos que fp16 moderno" a escala deployable es una **proyección** que combina (a) nuestra curva Wraith medida a 186M, (b) los regímenes publicados de BitNet y fp16 en producción (ver tabla de procedencia arriba), y (c) la hipótesis de que la eficiencia informacional por token escala con el tamaño del modelo sin cambio de régimen. Esta hipótesis **no está validada aún**; Wraith-2B a 100B tokens (Sec. 6.3) es el primer test empírico de esta hipótesis.
